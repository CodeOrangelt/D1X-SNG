...
//--
//change for each bytes added, which is the value after len += in net_udp.c, check with Arne, or read chat receipts. \/
//--
#define UPID_GAME_INFO_SIZE			(6 + 4*2 + 372 + (NETGAME_NAME_LEN+1) + (MISSION_NAME_LEN+1) + ((MAX_PLAYERS+4)*(CALLSIGN_LEN+1)) + 20*12)
