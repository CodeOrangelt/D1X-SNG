...
int do_powerup(object* obj)
{
		...
		return 0;
	{
		...
		case POW_SHIELD_BOOST:
			...
			break;
		case POW_LASER:
			if (Players[Player_num].laser_level >= MAX_LASER_LEVEL) //already have maxed lasers
			{
				Players[Player_num].laser_level = MAX_LASER_LEVEL;
				HUD_init_message(HM_DEFAULT | HM_REDUNDANT | HM_MAYDUPL, TXT_MAXED_OUT, TXT_LASER);
			}
			//come back to this fucking stupid shit code. - code
			if (!(Players[Player_num].laser_level >= MAX_LASER_LEVEL)) //no lasers or not all of them
			{
				...
				else if (!(Netgame.CTF | Netgame.StaticPowerups) && (Newdemo_state == ND_STATE_RECORDING))
				{
		...
		case POW_QUAD_FIRE:
			if (Netgame.CTF | Netgame.StaticPowerups & Players[Player_num].flags && PLAYER_FLAGS_QUAD_LASERS) //CTF + already has quads
			{
				HUD_init_message(HM_DEFAULT | HM_REDUNDANT | HM_MAYDUPL, "%s %s!", TXT_ALREADY_HAVE, TXT_QUAD_LASERS);
				only_sound = used;
				used = 0;
			}
			else if (Netgame.CTF | Netgame.StaticPowerups && !(Players[Player_num].flags & PLAYER_FLAGS_QUAD_LASERS))// CTF + doesn't have quads.
			{
				Players[Player_num].flags |= PLAYER_FLAGS_QUAD_LASERS;
